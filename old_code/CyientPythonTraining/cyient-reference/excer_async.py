...
async def load(url, session, sleep=0.1,debug=True, dir=r".", createFile=True):
        #* write this code 
        #to get similar output of threading code 
...
async def run(baseUrl,sleep,debug,dir,createFile,maxDepth,executor,loop,HACK):
    ...
    #* parse and get all links from base site ie call runOne and get urls 
    ...
    async with aiohttp.ClientSession() as session:
        while depth < maxDepth:
            #*call runOne with gather and then wait for to get list of list in res var
            #flatten 
